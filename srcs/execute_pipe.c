...
/*   Updated: 2022/07/11 00:45:25 by wchae            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

void	do_final_pipe_child(t_env *env, t_cmd *cmd, int prev_fd)
{
...
void	do_pipe_child(t_env *env, t_cmd *cmd, int pipe_fd[2], int prev_fd)
{
	signal(SIGINT, SIG_DFL);
	signal(SIGQUIT, SIG_DFL);
	if (prev_fd != -1)
